...
ggsave("stacked_barplot.png", plot = stacked_barplot, width = 10, height = 8, dpi = 300)
...
# 5. Emerging Technology Timeline
timeline_plot <- df %>%
  filter(`Year founded` >= 2010) %>%
  group_by(`Year founded`, `Technology Focus`) %>%
  summarise(n = n(), .groups = "drop") %>%
  ggplot(aes(x = `Year founded`, y = n, color = `Technology Focus`)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 3) +
  scale_color_brewer(palette = "Set1") +
  labs(title = "Evolution of Technological Adoption (2010-2023)",
       subtitle = "Precision fermentation shows exponential growth",
       x = "", y = "New Companies Founded") +
  theme_minimal() +
